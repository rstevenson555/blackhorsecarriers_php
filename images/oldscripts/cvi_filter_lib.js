/**
 * cvi_filter_lib.js 1.5 (02-Jul-2009) (c) by Christian Effenberger 
...
var cvi_filter = {version : 1.5, released : '2009-07-02 15:15:00', defaultF : null, defaultM : null, defaultS : -1, 
	add : function(obj,img,opts,w,h) {
		...
		if(obj&&obj.tagName.toUpperCase()=="CANVAS") { 
			if(obj.getContext) {var s,a,d,r,g,b,p,c,f,i,j,k,l,m,n,o,q,t,u,v,x=0,y=0,z=0,cb=false,hsb,rgb,ctx,bcx,defopts,proper,prepared; 
				...
				if(bcx&&prepared&&f!=null&&w>0&&h>0) {w+=4; h+=4; if(f=="convolve"&&(typeof m==='object')&&m!=null||(typeof cvi_matrix[f]==='object')) {s=ctx.getImageData(x,y,w,h); a=s.data;  
					...
					}else if(f=="smooth") {bcx.drawImage(obj,0,0,w,h,0,0,w-4,h-4); v=(c>0?Math.min(10,Math.max(1,c)):1); t=Math.round(v*5); b=Math.round(w*.75); q=Math.round(h*.75); for(i=0;i<t;i++) {r=Math.max(2,Math.round(b-(2*i))); g=Math.max(2,Math.round(q-(2*i))); bcx.drawImage(obj,0,0,w,h,0,0,r,g); ctx.drawImage(img,0,0,r,g,0,0,w,h);}bcx.drawImage(obj,0,0,w,h,0,0,w-4,h-4);				
